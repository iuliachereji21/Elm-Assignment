...
import Model.Date as Date exposing (Date, Month, compare, view)
    ...
    let 
        compareEnd = 
            case (intA.end, intB.end) of
                ...
                (Just d1, Nothing) -> LT
    in
    chainCompare compareEnd (Date.compare intA.start intB.start)
...
view (Interval intA) =
    let
        ...
        isLength =
            case length (Interval intA) of
                Just (y, m) -> [text <| (String.fromInt y) ++ " "++ (String.fromInt m)]
                _ -> [text ""] 
    
    in
    div [class "interval", style "font-size" "40%"] 
    [ h1 [class "interval-start"] [Date.view intA.start]
    , text " - "
    , h1 [class "interval-end"] [displayEnd]
    , h1 [class "interval-length"] isLength
    ]
